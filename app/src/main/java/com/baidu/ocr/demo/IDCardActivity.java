            ...
            public void onResult(IDCardResult result) {
                if (result != null) {
                    if (idCardSide.equals(IDCardParams.ID_CARD_SIDE_FRONT)){
                        ...
                        bundle.putString("name",result.getName().getWords());
                        bundle.putString("gender",result.getGender().getWords());
                        bundle.putString("birthday",result.getBirthday().getWords());

                        bundle.putString("ethnic",result.getEthnic().getWords());/*民族*/
                        bundle.putString("idNumber",result.getIdNumber().getWords());

                        bundle.putString("address",result.getAddress().getWords());
                        Intent intent = new Intent();
                        intent.putExtras(bundle);
                        intent.setClass(IDCardActivity.this,IdCardFrontActivity.class);
                        startActivity(intent);
                    }else {
                        Intent intent = new Intent();
                        intent.setClass(IDCardActivity.this,IDActivity.class);

                        String expiryDate = String.valueOf(result.getSignDate())+"--"+String.valueOf(result.getExpiryDate());
                        String adr = String.valueOf(result.getIssueAuthority());
                        intent.putExtra("bir",expiryDate);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_CAMERA && resultCode == Activity.RESULT_OK) {
            if (data != null) {
                ...
                Log.i("cccccccccccc","contentType=="+contentType);
